...
The `cc-tutor` skill SHALL be the only skill where `disableModelInvocation` is explicitly set to `false`. All other skills SHALL have `disableModelInvocation: true` or omit the field (which defaults to `true`). This ensures cc-tutor's description is the only one consuming Claude Code's skill description budget (~8,000 chars), guaranteeing it is never truncated.
